...
def request(query, params):
    ...
    # PATCH: Yahoo now returns HTTP 500 ("INKApi Error") when the empty
    # `iscqry=` parameter is sent. Upstream still sends it on page 1, which
    # makes the engine fail on every first-page query. Omitting it returns
    # normal results, so the page-1 branch is dropped entirely.
    if params['pageno'] >= 2:
